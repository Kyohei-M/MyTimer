...
namespace MyTimer
{
    ...
    class CountDownPageViewModel : BindableBase
    {
        ...
        // 一時停止時間用のストップウォッチ
        private System.Diagnostics.Stopwatch _stopwatch_pause = new System.Diagnostics.Stopwatch();

        private bool isPausing = false;

        private string _pauseText = "一時停止";
        public string PauseText
        {
            get { return _pauseText; }
            private set { SetProperty(ref _pauseText, value); }
        }
        ...
        private void ChangePauseState()
        {
            if (isPausing)
            {
                _stopwatch_pause.Stop();
                PauseText = "一時停止";
            }
            else
            {
                _stopwatch_pause.Start();
                PauseText = "再開";
            }
        ...
        public CountDownPageViewModel()
        {
            ...
            PauseCommand = new Command(ChangePauseState);
        ...
        {
            var time = Math.Max(0, (TimerSettings.Instance.CountMilliseconds - _stopwatch.ElapsedMilliseconds + _stopwatch_pause.ElapsedMilliseconds));
